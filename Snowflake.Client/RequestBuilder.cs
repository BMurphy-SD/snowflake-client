...
namespace Snowflake.Client
{
    internal class RequestBuilder
    {
        ...
        internal HttpRequestMessage BuildProcedureQueryRequest(string sql, object sqlParams, bool describeOnly)
        {
            var queryUri = BuildQueryUrl();
            string SPParams = "";
            var requestBody = new QueryRequest()
            ...
            if(requestBody.Bindings != null) SPParams = string.Join(",", requestBody.Bindings.Keys.Select(key => $":{key}"));
            requestBody.SqlText = $"CALL {requestBody.SqlText}({SPParams});";
