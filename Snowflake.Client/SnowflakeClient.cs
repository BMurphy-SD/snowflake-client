...
namespace Snowflake.Client
{
    public class SnowflakeClient : ISnowflakeClient
    {
        ...
        public async Task<T> ExecuteProcedureScalar<T>(string sql, object sqlParams = null, CancellationToken ct = default)
        {
            var response = await QueryProcedureInternalAsync(sql, sqlParams, false, ct).ConfigureAwait(false);
        ...
        public async Task<long> ExecuteProcedure(string sql, object sqlParams = null, CancellationToken ct = default)
        {
            var response = await QueryProcedureInternalAsync(sql, sqlParams, false, ct).ConfigureAwait(false);
            var affectedRows = SnowflakeUtils.GetAffectedRowsCount(response);
        ...
        public async Task<IEnumerable<T>> QueryAsync<T>(string sql, object sqlParams = null, CancellationToken ct = default)
        {
            ...
            var result = SnowflakeDataMapper.MapToObject<T>(response.Data.RowType, rowSet);
            //var result2 = SnowflakeDataMapper.MapTo<T>(response.Data.RowType, rowSet);
            return result;
        }
        public async Task<IEnumerable<T>> QueryProcedure<T>(string sql, object sqlParams = null, CancellationToken ct = default)
        {
            var response = await QueryProcedureInternalAsync(sql, sqlParams, false, ct).ConfigureAwait(false);
        ...
        public IEnumerable<T> Query<T>(string sql, object sqlParams = null, CancellationToken ct = default)
        {
            var response = QueryInternalAsync(sql, sqlParams, false, ct).Result;
            ...
            if (response.Data.Chunks != null && response.Data.Chunks.Count > 0)
            {
                ...
                var parsedRowSet = ChunksDownloader.DownloadAndParseChunksAsync(chunksDownloadInfo, ct).Result;
                rowSet.AddRange(parsedRowSet);
            }
            var result = SnowflakeDataMapper.MapToObject<T>(response.Data.RowType, rowSet);
            //var result2 = SnowflakeDataMapper.MapTo<T>(response.Data.RowType, rowSet);
        ...
        public async Task<T> SingleAsync<T>(string sql, object sqlParams = null, CancellationToken ct = default)
        {
            var response = await QueryInternalAsync(sql, sqlParams, false, ct).ConfigureAwait(false);
            return SnowflakeDataMapper.MapToObject<T>(response.Data.RowType, response.Data.RowSet).FirstOrDefault();
        }
        public async Task<T> SingleProcedure<T>(string sql, object sqlParams = null, CancellationToken ct = default)
        {
            var response = await QueryProcedureInternalAsync(sql, sqlParams, false, ct).ConfigureAwait(false);
            return SnowflakeDataMapper.MapToObject<T>(response.Data.RowType, response.Data.RowSet).FirstOrDefault();
        }
        public T Single<T>(string sql, object sqlParams = null, CancellationToken ct = default)
        {
            var response = QueryInternalAsync(sql, sqlParams, false, ct).Result;
            return SnowflakeDataMapper.MapToObject<T>(response.Data.RowType, response.Data.RowSet).FirstOrDefault();
        }
        ...
        private async Task<QueryExecResponse> QueryProcedureInternalAsync(string sql, object sqlParams = null, bool describeOnly = false, CancellationToken ct = default)
        {
            if (_snowflakeSession == null)
            {
                await InitNewSessionAsync(ct).ConfigureAwait(false);
            ...
            var queryRequest = _requestBuilder.BuildProcedureQueryRequest(sql, sqlParams, describeOnly);
            var response = await _restClient.SendAsync<QueryExecResponse>(queryRequest, ct).ConfigureAwait(false);
            ...
            if (response.Code == 390112)
            {
                ...
                // A new instance of HttpQueryRequest should be created for every request
                queryRequest = _requestBuilder.BuildProcedureQueryRequest(sql, sqlParams, describeOnly);
                response = await _restClient.SendAsync<QueryExecResponse>(queryRequest, ct).ConfigureAwait(false);
            }
        ...
        private QueryExecResponse QueryInternal(string sql, object sqlParams = null, bool describeOnly = false, CancellationToken ct = default)
        {
            if (_snowflakeSession == null)
            {
                InitNewSessionAsync(ct).ConfigureAwait(false);
            }

            var queryRequest = _requestBuilder.BuildQueryRequest(sql, sqlParams, describeOnly);
            var Taskresponse = _restClient.SendAsync<QueryExecResponse>(queryRequest, ct).ConfigureAwait(true);
            var response = Taskresponse.GetAwaiter().GetResult();

            // Auto renew session, if it's expired
            if (response.Code == 390112)
            {
                RenewSessionAsync(ct).ConfigureAwait(false);

                // A new instance of HttpQueryRequest should be created for every request
                queryRequest = _requestBuilder.BuildQueryRequest(sql, sqlParams, describeOnly);
                Taskresponse =  _restClient.SendAsync<QueryExecResponse>(queryRequest, ct).ConfigureAwait(false);
            ...
            if (response.Code == 333334 || response.Code == 333333)
            {
                Taskresponse =  RepeatUntilQueryCompleted(response.Data.GetResultUrl, ct).ConfigureAwait(false);
                response = Taskresponse.GetAwaiter().GetResult();
            }
